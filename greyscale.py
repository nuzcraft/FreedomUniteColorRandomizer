...
if __name__=='__main__':
    ...
    for idx, texture in enumerate(imgs):
        ...
        # new_img = Image.fromarray(np.dot(arr[...,:4], [0.299, 0.587, 0.144, 1.0]).astype('uint8'), 'RGBA')
        # new_name = files[idx].replace("palette", "grey_palette")
        # new_img.save(files[idx])
        # new_img.save(new_name)
        new_img = ImageOps.grayscale(texture)
        new_name = files[idx].replace("palette", "grey_palette")
        new_img.save(new_name)
